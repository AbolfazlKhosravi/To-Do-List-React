...
const Navbar = () => {
  ...
  const serchHandler=(e)=>{
   ...
   dispatch(multipleFilterAsynchTodos({title:e.target.value,selected:select.value}))
  }
  if (data&&data.length==0&&!inputValue&&!select) return <h2 className="text-white mb-5 bg-gray-800 px-8 py-2 rounded-sm font-bold">set today Todos</h2>;
  if(data)return (
    <header className="flex flex-col gap-2 items-center mb-4 md:mb-8 md:gap-3">
      <div >
        <p className="text-white text-sm md:text-[1rem]">Undone work :<span className="ml-1" id="undoneWork">{unDownHandler()}</span></p>
      </div>
